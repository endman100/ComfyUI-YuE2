...
3. **YuE2 Generate Semantic** returns standard `CONDITIONING` plus the semantic payload used to size the latent.
4. **YuE2 Empty Latent** returns a standard `LATENT`, split at the official context boundaries.
5. **YuE2 Explicit Midpoint** returns a standard `SAMPLER` implementing the official chunk-major RK2 solver.
6. **YuE2 Linear Schedule** returns standard `SIGMAS`, exactly `1 → 0` with 32 steps by default.
7. **YuE2 Decode** converts the sampled standard `LATENT` to 48 kHz stereo `AUDIO` without the amplitude normalization performed by ComfyUI's generic audio VAE decoder.
...
RandomNoise ──────────────────────────────────────────────────────────┤
YuE2 Explicit Midpoint ───────────────────────────────────────────────┤
YuE2 Linear Schedule ─────────────────────────────────────────────────┤
YuE2 Empty Latent ────────────────────────────────────────────────────┴→ SamplerCustomAdvanced
...
- `yue2_abc_cover_rearrangement`: uses editable supplied ABC notation for a cover or rearrangement.
...
Tested against ComfyUI 0.35.0's stable `comfy_api.v0_0_2`, frontend 1.51.10, Python 3.12, PyTorch 2.14.0 + CUDA 13.0, and the official YuE2 inference 0.1.5 compatibility build on an NVIDIA RTX 5090. The node package requires ComfyUI 0.35.0 or newer.
